...
async def get_all_users(session: AsyncSession = Depends(get_async_session)):
	...
	rezult_data = [GetDataUsers(id=user[0].id, email=user[0].email, username=user[0].username,
								role_id=user[0].role_id, is_verified=user[0].is_verified) for user in rez_query]
	if not rezult_data:
...
async def update_my_user(values: UpdateUser, session: AsyncSession = Depends(get_async_session),
						 user=Depends(current_user)):
	query = select(User).where(User.id == user.id)
	rez_query = await session.execute(query)
	if not rez_query.fetchall()[0][0].username:
		yield HTTPException(status_code=403, detail="Доступ запрещен")
	stmt = update(User).where(User.id == user.id).values(values.dict())
