...
def get_email_report_video(user_email: str, last_five_video: List[str]):
	...
	email.set_content(
		...
		f'<h2 style="color: red;">Вот они: {", ".join([str(x) for x in [*last_five_video]])}</h2>'
		'</div>',
		subtype='html'
	)
	return email
...
@celery.task
def send_email_report_last_video(user_email: str, last_five_video: List[str]):
	email = get_email_report_video(user_email, last_five_video)
